...
func (this *Signer) fetchURL(fetch *url.URL, serveHTTPReq *http.Request) (*http.Request, *http.Response, *util.HTTPError) {
	...
	// X-Forwarded-For is taken net/http/httputil/reverseproxy.go
        if clientIP, _, err := net.SplitHostPort(serveHTTPReq.RemoteAddr); err == nil {
		if prior, ok := serveHTTPReq.Header["X-Forwarded-For"]; ok {
